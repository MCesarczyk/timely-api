...
export class PeriodController {
  ...
  async getPeriods(
    ...
    const data = await this.periodService.getPeriods(
      { todoId, type },
      perPage,
      page,
    );
  ...
  @Get(':id')
  getPeriodById(@Param('id') id: number): Promise<Period | null> {
  ...
  createPeriod(@Body() newPeriod: Period): Promise<Period> {
    if (!newPeriod.todoId) {
      throw new Error('todoId is required to create a period');
    }
  ...
  updatePeriod(
    ...
    if (!updatePeriod.todoId) {
      throw new Error('todoId is required to update a period');
    }
